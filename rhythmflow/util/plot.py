...
def plot_batch(batch, name=None):
    ...
    for b in range(batch.shape[0]):
        row = int(b / 4)
        col = int(b % 4)
        r_p = row * batch.shape[2] + row
        c_p = col * batch.shape[3] + col
        img[r_p : (r_p + batch.shape[2]), c_p : (c_p + batch.shape[3])] = batch[
            b
        ].squeeze()
...
def plot_batch_detailed(batch, params, name=None):
    ...
    for b in range(batch.shape[0]):
        axes[b, 0].bar(
            np.linspace(0, params[b].shape[0] - 1, params[b].shape[0]), params[b]
        )
        axes[b, 1].imshow(batch[b], aspect="auto")
        for w in range(4):
            axes[b, w + 2].plot(
                batch[b, int(float((w + 1.0) / 5) * batch[b].shape[0])].numpy()
            )
    if name is not None:
...
def compare_batch_detailed(
    ...
    for b in range(batch.shape[0]):
        ...
        axes[b, 2].bar(
            np.linspace(0, params[b].shape[0] - 1, params[b].shape[0]),
            params[b],
            color="b",
        )
        if params is not None:
            axes[b, 3].bar(
                np.linspace(0, params[b].shape[0] - 1, params_r[b].shape[0]),
                params_r[b],
                color="r",
            )
        if wave is not None:
