...
from ..flows import NormalizingFlow, Flow
from ..flows import GaussianDiag, sum_dims
from ..flows import SigmoidFlow
...
class VariationalLambda(nn.Module):
    ...
    def __init__(
        self, latent_size, init_val=1e-1, init_type="fixed", variational=True, **kwargs
    ):
        ...
        self.p_mean = torch.zeros(
            latent_size
        ).detach()  # nn.Parameter(torch.zeros(latent_size))
        self.p_var = torch.ones(
            latent_size
        ).detach()  # nn.Parameter(torch.ones(latent_size))
        if init_type == "rand":
            self.q_var = nn.Parameter(
                torch.ones(latent_size)
                * (torch.abs(torch.randn(latent_size)) * init_val)
            )
        # Base distribution
        self.base_dist = MultivariateNormal(
            torch.zeros(latent_size), torch.eye(latent_size)
        )
    ...
    def gaussian_likelihood(self, x, mean, logs):
        ...
        return -0.5 * (logs * 2.0 + ((x - mean) ** 2) / torch.exp(logs * 2.0) + log_2pi)
    ...
    def get_values(self, n_vals):
        samples = torch.rand(n_vals, self.latent_size).to(
            self.q_var.device
        )  # self.base_dist.sample((n_vals, ))
...
class FlowPredictor(nn.Module):
    ...
    def kullback_leibler(self, f_mu, f_sig, g_mu, g_sig):
        ...
        means = f_mu - g_mu
        div = 0.5 * (
            var_2 - var_1 + var_frac + torch.sum((means ** 2) / (var_2 + 1e-9))
        )
        return -div
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(FlowTransform, self).__init__(
            latent_size, blocks, flow_length, amortize, eps_var, var_type, **kwargs
        )
...
class FlowKLFull(FlowPredictor):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(FlowKLFull, self).__init__(
            latent_size, blocks, flow_length, amortize, eps_var, var_type, **kwargs
        )  # Prior distribution
        # Prior on y
...
class FlowAmortizedPredictor(FlowPredictor):
    def __init__(
        ...
        **kwargs
        ...
        super(FlowAmortizedPredictor, self).__init__(
            latent_size, blocks, flow_length, "ext", eps_var, var_type, **kwargs
        )
    ...
    def amortization_net(self, dim, n_hidden, out, nlayers=2, activation=nn.ReLU):
        ...
        for l in range(nlayers):
            cur_n = nn.Linear(
                (l == 0) and dim or n_hidden, (l == nlayers - 1) and out or n_hidden
            )
            nn.init.xavier_uniform_(cur_n.weight.data, gain=0.1)
...
class FlowCDE(FlowAmortizedPredictor):
    def __init__(
        ...
        **kwargs
        ...
        super(FlowCDE, self).__init__(
            latent_size, blocks, flow_length, "ext", eps_var, var_type, **kwargs
        )
        # Parameters of flow
        self.n_params = self.flow.n_parameters()
        # Variational approximation to parameter posterior
        self.flow_params = VariationalLambda(
            self.n_params, init_val=eps_var * 1e-2, init_type="fixed"
        )
...
class FlowExternal(FlowAmortizedPredictor):
    def __init__(
        ...
        **kwargs
        ...
        super(FlowExternal, self).__init__(
            latent_size, blocks, flow_length, "ext", eps_var, var_type, **kwargs
        )
    ...
    def log_prob(self, x, y):
        # Sample from the base distribution
        samples = (
            torch.rand_like(x).detach().to(self.var)
        )  # self.base_dist.sample((x.shape[0],))
        samples = (self.var * samples) + self.mean
        # Compute amortization parameters
        params = self.h_net(x)
        # Amortize the flow
...
class FlowPosterior(FlowPredictor):
    def __init__(
        ...
        **kwargs
        ...
        super(FlowPosterior, self).__init__(
            latent_size, blocks, flow_length, amortize, eps_var, var_type, **kwargs
        )
        # Prior on y
    ...
    def log_prob(self, x, y):
        ...
        for mc_it in range(4):
            ...
            var_y, var_loss = self.var_y.sample(y.shape[0])
            # Compute posterior
            reg_loss = self.gaussian_likelihood(
                z_out, y, var_y
...
class StochasticFlowParameters(nn.Module):
    ...
    def sample(self, n=1, context=None):
        ...
        for s in range(len(self.samplers)):
            ...
            self.logdets.append(
                torch.mean(torch.cat(cur_logdet, dim=1), dim=1) + x_logdet
            )
