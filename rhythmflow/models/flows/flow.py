...
class NormalizingFlow(nn.Module):
    ...
    def __init__(
        self, dim, blocks, flow_length, final_block=None, density=None, amortized="none"
    ):
    ...
    def parameters_network(self, nin, nout, nlayers=2, nhidden=64, activation=nn.ReLU):
        ...
        for l in range(nlayers):
            cur_n = nn.Linear(
                (l == 0) and nin or nhidden, (l == nlayers - 1) and nout or nhidden
            )
...
class GenerativeFlow(NormalizingFlow):
    ...
    def __init__(
        ...
        super(GenerativeFlow, self).__init__(
            dim, blocks, generative_layers, target_density, "none"
        )
        biject = []
        self.n_params = []
        self.output_shapes = []
    ...
    def forward(self, z):
        ...
        # Add noise to input
        z = z + torch.uniform(
            mean=torch.zeros_like(z), std=torch.ones_like(z) * (1.0 / 256.0)
        )
        # Include noise in the log_dets
        self.log_det.append(torch.zeros_like(z[:, 0, 0, 0]) + (-np.log(256.0) * dim_in))
