    ...
    def forward(self, x, meta=None):
        ...
        if meta is not None:
            ...
            if target_obs.eq(0).sum() > 0:
                ...
                neg_loss = (
                    self.gaussian_likelihood(
                        z_out_obs[target_obs.eq(0), 0],
                        self.p_mu_0.repeat(target_obs.eq(0).sum()),
                        var_0,
                    ).sum()
                    * 10
                )
                neg_loss += self.gaussian_likelihood(
                    z_out_obs[target_obs.eq(0), 1:], self.p_mu[1:], self.var[1:]
                ).mean()
                # print('Negative')
                # print(var_0)
            ...
            if target_obs.eq(1).sum() > 0:
                ...
                pos_loss = (
                    self.gaussian_likelihood(
                        z_out_obs[target_obs.eq(1), 0],
                        self.p_mu_1.repeat(target_obs.eq(1).sum()),
                        var_1,
                    ).sum()
                    * 10
                )
                pos_loss += self.gaussian_likelihood(
                    z_out_obs[target_obs.eq(1), 1:], self.p_mu[1:], self.var[1:]
                ).mean()
                # print('Positive')
                # print(var_1)
                # print(z_out_obs[target_obs.eq(1), 0])
            # Compute objective over positive tags
            unk_loss = self.gaussian_likelihood(
                z_out[unknown, :], self.p_mu, self.var
            ).mean()
