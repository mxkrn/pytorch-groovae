...
class SylvesterFlow(Flow):
    ...
    def __init__(self, dim, num_ortho_vecs=16, steps=50, amortized="none"):
        ...
        self._eye = (
            torch.eye(self.num_ortho_vecs, self.num_ortho_vecs).unsqueeze(0).detach()
        )
        self.dim = dim
        self.zk = []
        self.r1 = []
    ...
    def set_parameters(self, params, batch_dim):
        ...
        self.r1 = (
            params[:, self.dim : (self.dim + self.num_ortho_vecs ** 2)]
            .repeat(batch_dim, 1)
            .view(-1, self.num_ortho_vecs, self.num_ortho_vecs)
        )
        self.r2 = (
            params[
                :,
                (self.dim + self.num_ortho_vecs ** 2) : (
                    self.dim + (2 * self.num_ortho_vecs ** 2)
                ),
            ]
            .repeat(batch_dim, 1)
            .view(-1, self.num_ortho_vecs, self.num_ortho_vecs)
        ...
        self.q_ortho = (
            params[
                ...
                (self.dim + (2 * self.num_ortho_vecs ** 2)) : (
                    self.dim
                    + (2 * self.num_ortho_vecs ** 2)
                    + (self.dim * self.num_ortho_vecs)
                ),
            ]
            .repeat(batch_dim, 1)
            .view(-1, self.dim, self.num_ortho_vecs)
        )
        self.b = (
            params[
                :,
    ...
    def n_parameters(self):
        ...
        return (
            (self.dim)
            + (2 * self.num_ortho_vecs ** 2)
            + ((self.dim + 1) * self.num_ortho_vecs)
        )
...
class TriangularSylvesterFlow(SylvesterFlow):
    ...
    def __init__(self, dim, num_ortho_vecs=16, steps=50, amortized="none"):
        ...
        """
        super(TriangularSylvesterFlow, self).__init__(
            dim, num_ortho_vecs, steps, amortized
        )
        self.num_ortho_vecs = num_ortho_vecs
    ...
    def set_parameters(self, params, batch_dim):
        ...
        diag1 = params[:, (self.dim ** 2) : ((self.dim ** 2) + self.dim)].view(
            -1, self.dim
        )
        diag2 = params[
            :, ((self.dim ** 2) + self.dim) : ((self.dim ** 2) + (2 * self.dim))
        ].view(-1, self.dim)
        self.b = params[:, ((self.dim ** 2) + (2 * self.dim)) :].view(
            -1, 1, self.dim
        )  # .repeat(batch_dim, 1).view(-1, 1, self.dim)
