...
class AE(nn.Module):
    def __init__(self, encoder, decoder, encoder_dims, latent_dims):
        ...
        self.to_latent = nn.Linear(encoder_dims, latent_dims)
        self.from_latent = nn.Linear(latent_dims, encoder_dims)
    ...
    def forward(self, x, hidden):
        x, hidden = self.encode(x, hidden)  # Encode the inputs
        z_tilde, z_loss = self.regularize(x)  # Potential regularization
        y = self.from_latent(z_tilde)
        x_tilde = self.decode(y, hidden)  # Decode the samples
...
class RegressionAE(nn.Module):
    ...
    def train_epoch(self, loader, loss_params, optimizer, args):
        ...
        for (x, y, _, _) in loader:
            ...
            # Final loss
            b_loss = (rec_loss + (args.beta * z_loss) + (args.gamma * reg_loss)).mean(
                dim=0
            )
            # Perform backward
            optimizer.zero_grad()
...
class DisentanglingAE(RegressionAE):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(DisentanglingAE, self).__init__(
            ae_model,
            latent_dims,
            regression_dims,
            recons_loss,
    ...
    def train_epoch(self, loader, loss_params, optimizer, args):
        ...
        for (x, y, meta, _) in loader:
            ...
            x, y, meta = (
                x.to(args.device, non_blocking=True),
                y.to(args.device, non_blocking=True),
                meta.to(args.device, non_blocking=True),
            )
            ...
            z_tilde, z_loss = self.ae_model.latent(x, z_params)
            # Disentangling part
            z_tilde, dis_loss = self.disentangling(
                z_tilde, (meta, target, observed, unknown)
            )
            # Decode the samples
