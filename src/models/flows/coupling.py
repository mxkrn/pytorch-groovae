...
class AffineCouplingFlow(Flow):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.k = dim // 2
        if input_dims == 1:
            self.g_mu = self.transform_net(
                self.k, self.k, n_hidden, n_layers, activation
            )
            self.g_sig = self.transform_net(
                self.k, self.k, n_hidden, n_layers, activation
            )
    ...
    def transform_net(self, nin, nout, nhidden, nlayer, activation):
        ...
        for l in range(nlayer):
            net.append(
                nn.Linear(
                    l == 0 and nin or nhidden, (l == nlayer - 1) and nout or nhidden
                )
            )
            if l < nlayer - 1:
    ...
    def transform_conv(self, nin, nout, nhidden):
        return nn.Sequential(
            ...
            Conv2d(nhidden, nhidden, kernel_size=[1, 1]),
            nn.ReLU(inplace=False),
...
class MaskedCouplingFlow(Flow):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.mask = (
            mask or torch.cat((torch.ones(self.k), torch.zeros(self.k))).detach()
        )
        self.init_parameters()
        self.bijective = True
